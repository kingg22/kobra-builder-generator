...
class GoToBuilderActionHandlerTest {
    @InjectMocks
    private lateinit var builderActionHandler: GoToBuilderActionHandler
    ...
    @Mock
    private lateinit var popupListFactory: GoToBuilderPopupListFactory

    @Mock
    private lateinit var popupDisplayer: GoToBuilderPopupDisplayer
    ...
    @Test
    fun shouldNavigateToBuilderIfItExistsAndInvokedInsideNotBuilderClass() {
        // given
        psiHelper.`when`<PsiClass> { PsiHelper.getPsiClassFromEditor(editor, project) }
            .thenReturn(psiClass)
        builderVerifier.`when`<Boolean> { BuilderVerifier.isBuilder(psiClass) }
            .thenReturn(false)
        builderFinder.`when`<Any?> { BuilderFinder.findBuilderForClass(psiClass) }
        ...
        // then
        psiHelper.verify { navigateToClass(builderClass) }
    }

    @Test
    fun shouldNavigateToNotBuilderClassIfItExistsAndInvokedInsideBuilder() {
        // given
        psiHelper.`when`<PsiClass> { PsiHelper.getPsiClassFromEditor(editor, project) }
            .thenReturn(builderClass)
        builderVerifier.`when`<Boolean> { BuilderVerifier.isBuilder(builderClass) }
            .thenReturn(true)
        builderFinder
        ...
        // then
        psiHelper.verify { navigateToClass(psiClass) }
    }

    @Test
    fun shouldDisplayPopupWhenBuilderNotFoundAndInvokedInsideNotBuilderClass() {
        // given
    ...
    private fun verifyNothingIsDone(psiHelper: MockedStatic<PsiHelper>) {
        psiHelper.verify({ navigateToClass(any()) }, never())
